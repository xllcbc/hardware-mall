...
- **shop_order ↔ payment_record**: 一对多，一个订单可有多条支付/退款记录（`out_trade_no` 唯一）
...
CREATE TABLE `shop_order` (
    ...
    `status` TINYINT NOT NULL DEFAULT 1 COMMENT '订单状态：1-待付款，2-待发货，3-已发货，4-已完成，5-已取消，6-退款中，7-已退款，8-退款申请中，9-退款失败',
...
CREATE TABLE `payment_record` (
    ...
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0-待支付，1-已支付，2-已关闭，3-已退款，4-退款中',
...
**说明**：
- 保存微信支付/退款流水，`out_trade_no`（商户订单号）全局唯一，保证微信侧幂等
- 退款流程：`1-已支付` → CAS 置 `4-退款中`（claim-first）→ 微信退款回调/定时对账 → 成功置 `3-已退款`
- 退款成功后由回调/对账任务将订单推进到 `7-已退款` 并回补库存
...
| 49 | payment_record | PRIMARY | 主键 | id | 主键 |
| 50 | payment_record | uk_out_trade_no | 唯一 | out_trade_no | 商户订单号唯一 |
| 51 | payment_record | idx_order_id | 普通 | order_id | 订单支付记录 |
...
1. 支付记录保存在 `payment_record`，`out_trade_no` 全局唯一，保证微信侧幂等
2. 支付状态：0-待支付，1-已支付，2-已关闭，3-已退款，4-退款中
3. 退款采用 claim-first：先 CAS 将支付记录置「退款中」再调用微信退款，外呼结果未知时保持「退款中」并交由回调/对账收敛
4. 退款结果由微信退款回调与定时对账任务确认：成功置「已退款」，失败回退到可重试态（订单状态 9-退款失败）
5. 库存回补发生在退款确认成功（订单 6→7）时，避免过早回补导致超卖
...
项目使用 `init.sql` 单脚本建表，包含：

- 基础表：user、address、category、logistics、shop_order、order_item
- SPU/SKU 相关表：spu、sku、spec_template、spec_item、cart
...
> 真实商品数据（价格表导入）不入库版本控制，请通过运维脚本单独导入。

---

**文档状态**：已更新至 V2.1 · 2026-09-16
**更新内容**：补充 payment_record 支付记录表与订单发货/微信订单状态字段；订单状态补充 8-退款申请中、9-退款失败；修正初始化脚本说明
